...
base_lr_divs = [10., 100]
base_lr_mults = [100, 1000, 10000]

u_l1regs = [-100, -1000, -10000, -50000, -100000]
...
def memtest(mat_path, base_lr, base_lr_div, base_lr_mult, u_l1reg):
    tf.reset_default_graph()
    mat = loadmat(mat_path)
    y = mat['y'].astype(np.float32)
    y_est = mat['y_est'].astype(np.float32)
    z = mat['z'].astype(np.float32)
    smoeo = Smoe(z, kernels_per_dim, train_pis=True, start_batches=batches)
    results_path = "denoise_batch2/{mat_path}/base_lr_{base_lr}/" \
                   "base_lr_div_{base_lr_div}/base_lr_mult_{base_lr_mult}/" \
                   "u_l1reg_{u_l1reg}".format(mat_path=mat_path,
                                              base_lr=base_lr,
    ...
    optimizer3 = tf.train.AdamOptimizer(base_lr * base_lr_mult, beta1=0.1, beta2=0.9, epsilon=0.001)
    try:
        smoeo.train(training_iters, val_iter=validation_iterations, optimizer1=optimizer1, optimizer2=optimizer2,
    ...
    try:
        ...
        mses = smoeo.get_mses()
        losses = smoeo.get_losses()
        num_pis = smoeo.get_num_pis()
...
for mat_path, base_lr, base_lr_div, base_lr_mult, u_l1reg in product(mat_paths, base_lrs, base_lr_divs, base_lr_mults,
                                                                     u_l1regs):
    memtest(mat_path, base_lr, base_lr_div, base_lr_mult, u_l1reg)
